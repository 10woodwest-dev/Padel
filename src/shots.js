...
export const SHOTS = {
  ...
  lob: {
    label: 'Lob', contact: [0.2, 0.55, 1.2], speed: [8.5, 12.5],
    spinTop: -30, spinSide: 0, loft: 2.05, error: 0.8, stat: 'lob',
  ...
  blockVolley: {
    label: 'Block volley', contact: [0.45, 1.0, 1.95], speed: [6.5, 10.5],
  ...
  wallDefence: {
    label: 'Wall return', contact: [0.2, 0.6, 1.5], speed: [9.5, 14.5],
...
export function executeShot(ballState, {
  ...
  target.z += randGauss(sigma * 1.25) * 1; // depth errors slightly larger
  target.x = clamp(target.x, -COURT.halfWidth + 0.05, COURT.halfWidth - 0.05);
  // NOTE: depth is NOT re-clamped after the error — an overhit ball genuinely
  // sails long into the glass (fault), which is how errors should manifest.
  ...
  // --- net clearance sanity: if the solved arc clips the net, loft it ------
  // (skip for mishits — those are allowed to be ugly)
  for (let tries = 0; tries < 3 && !mishit; tries++) {
    const yAtNet = heightAtNetCrossing(start, launch.vel, launch.spin);
    if (yAtNet === null || yAtNet > netHeightAt(0) + 0.09) break;
    T *= 1.14;
...
// Coarse check of ball height when crossing z=0 (the net plane).
function heightAtNetCrossing(p0, v0, spin) {
  const dt = 1 / 120;
  const p = { ...p0 }, v = { ...v0 };
